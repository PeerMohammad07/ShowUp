...
import { ChevronLeft, Zap, Info, Loader2, Star, Clock } from "lucide-react";
...
export default function CreateResolution() {
    ...
    async function handleSubmit(formData: FormData) {
        ...
        startTransition(async () => {
            const result = await createResolution(formData) as any;
            if (result?.error) {
    ...
    return (
        <div className="min-h-screen bg-[#F8F9FA] dark:bg-[#0A0A0A] pb-20">
            ...
            <header className="h-16 flex items-center px-6 sticky top-0 bg-[#F8F9FA]/80 dark:bg-[#0A0A0A]/80 backdrop-blur-lg z-50">
                <Link href="/dashboard" className="p-2 -ml-2 text-neutral-500 dark:text-neutral-400">
                    <ChevronLeft className="w-6 h-6" />
                </Link>
                <h1 className="flex-1 text-center font-bold dark:text-white mr-8 text-sm">Create Resolution</h1>
            </header>
            ...
            <main className="px-6 space-y-8 pt-6 max-w-xl mx-auto">
                <form action={handleSubmit} className="space-y-8">
                    ...
                    <div className="space-y-3 relative">
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 space-y-2">
                                ...
                                <p className="text-[10px] text-neutral-400 -mt-1 font-medium">Where are you starting from? Describe the context.</p>
                                <textarea
                                    name="situation"
                                    required
                                    placeholder="e.g. I have been feeling sluggish and unmotivated..."
                                    rows={3}
                    ...
                    <div className="space-y-3 relative">
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 space-y-2">
                                ...
                                <p className="text-[10px] text-neutral-400 -mt-1 font-medium">What specific challenge do you need to overcome?</p>
                                <textarea
                                    name="task"
                                    required
                                    placeholder="e.g. I need to improve my physical energy levels..."
                                    rows={3}
                                    className="w-full p-6 bg-white dark:bg-[#121212] border border-neutral-100 dark:border-neutral-800 rounded-[2rem] focus:outline-none focus:ring-2 focus:ring-[#00D261] transition-all dark:text-white text-sm leading-relaxed"
                                />
                            </div>
                    ...
                    <input type="hidden" name="result" value="Improved health and mindset" />
                    ...
                    <div className="flex gap-4 pt-4">
                        ...
                        >
                            {isPending ? <Loader2 className="w-6 h-6 animate-spin" /> : "Save Resolution"}
                        </button>
                    </div>
                </form>
            </main>
